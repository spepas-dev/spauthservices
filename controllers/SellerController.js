...
exports.ADD_SELLER = asynHandler(async (req, res, next) => {
   ...
   if(!sellerRespObj)
       ...
       if(sellerRespObj.status != RESPONSE_CODES.SUCCESS){
        ...
        let newUserUpdate = await UtilityHelper.makeHttpRequest("POST",updateURL, userToReg);
     ...
     try {
       ...
       if(!newJob)
        ...
        if(newJob.status != RESPONSE_CODES.SUCCESS){
         ...
         let sellerRespObj = await UtilityHelper.makeHttpRequest("POST",updateURL, sellerDetails);
