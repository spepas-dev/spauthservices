...
exports.ADD_DELIVER = asynHandler(async (req, res, next) => {
    ...
    let sellerRespObj = await UtilityHelper.makeHttpRequest("POST",updateURL, reqBody);
    ...
    if(!sellerRespObj)
        ...
        if(sellerRespObj.status != RESPONSE_CODES.SUCCESS){
            return UtilityHelper.sendResponse(res, 200, sellerRespObj.message, sellerRespObj);
         }
...
exports.UPLOAD_IMAGE = asynHandler(async (req, res, next) => {
    ...
    //TODO Validate file to only accept PDF
     ...
     try {
        const result = await cloudinary.uploader.upload(req.file.path, {
            folder: 'DeliverImages', // Optional: Organize files in a folder
          });
       ...
       if(!newJob)
        ...
        if(newJob.status != RESPONSE_CODES.SUCCESS){
         ...
         if(Image_Type == "FRONT")
            {
                //front image upload
                deliverDetails.front_license_url = result.secure_url;
                deliverDetails.front_license_obj = result;

            }else{
                //back image upload
                deliverDetails.back_license_url = result.secure_url;
                deliverDetails.back_license_obj = result;
         ...
         let sellerRespObj = await UtilityHelper.makeHttpRequest("POST",updateURL, deliverDetails);
 ...
 exports.ADD_VEHICLE = asynHandler(async (req, res, next) => {
    ...
    let sellerRespObj = await UtilityHelper.makeHttpRequest("POST",updateURL, body);
...
exports.UPLOAD_VEHICLE_IMAGE = asynHandler(async (req, res, next) => {
    ...
    //TODO Validate file to only accept PDF
     ...
     try {
        ...
        var loginUrl = process.env.DB_BASE_URL +"deliver/vehicle-details/"+Vehicle_ID; 
        let newJob = await UtilityHelper.makeHttpRequest("GET",loginUrl);
       ...
       if(!newJob)
        ...
        if(newJob.status != RESPONSE_CODES.SUCCESS){
         ...
         if(Image_Type == "FRONT")
            {
                //front image upload
                deliverDetails.front_image_url = result.secure_url;
                deliverDetails.front_image_obj = result;

            }else{
                //back image upload
                deliverDetails.back_image_url = result.secure_url;
                deliverDetails.back_image_obj = result;
