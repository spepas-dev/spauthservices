...
exports.LOGIN = asynHandler(async (req, res, next) => {
  ...
  let newJob = await UtilityHelper.makeHttpRequest(
    "POST",
    loginUrl,
    requestData
  );
  ...
  if (newJob.data.verificationStatus != RESPONSE_CODES.SUCCESS) {
    ...
    let otpReqResp = await UtilityHelper.makeHttpRequest(
      "POST",
      otpUrl,
      otpReqObj
    );
    ...
    var resp = {
      ...
      message:
        "User verification is pending. An OTP has been sent to the user for authentication",
      data: resetID,
    };
    return UtilityHelper.sendResponse(res, 200, resp.message, resp);
  }
  ...
  const token = jwt.sign(tokenDetails, process.env.JWT_TOKEN, {
    expiresIn: "10h",
  });
  const refresh_token = jwt.sign(tokenDetails, process.env.JWT_TOKEN_REFRESH, {
    expiresIn: "20h",
  });
...
exports.REGISTER = asynHandler(async (req, res, next) => {
  ...
  var resp = {
    status: RESPONSE_CODES.SUCCESS,
    message:
      "User registered, a verification code has been sent to your phone number or email to complete the process",
    data: resetID,
    UserID: newJob.data.User_ID,
  };
  return UtilityHelper.sendResponse(res, 200, resp.message, resp);
});
...
exports.FORGOT_PASSWORD = asynHandler(async (req, res, next) => {
  ...
  var resp = {
    status: RESPONSE_CODES.SUCCESS,
    message:
      "A verification code has been sent to your phone number or email to complete the process",
    data: resetID,
  };
  return UtilityHelper.sendResponse(res, 200, resp.message, resp);
});
...
exports.REGISTER_USER_BY_ADMIN = asynHandler(async (req, res, next) => {
  ...
  console.log(
    "================coming to make a registration request================"
  );
